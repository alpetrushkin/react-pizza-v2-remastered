...
export const Home = () => {
  const categoryId = useSelector((state) => state.filter.c)
  ...
  React.useEffect(() => {
    ...
    fetch(
      `https://640aa2a181d8a32198ccd7e2.mockapi.io/pizzas?page=${currentPage}&limit=4${category}&sortBy=${sortBy}&order=${order}${search}`
    )
      .then((res) => res.json())
      .then((arr) => {
        setPizzas(arr);
